...
namespace cflat {
	...
	Instruction::Instruction(int opcode, Argument a0, Argument a1, Argument a2) : opcode(opcode), a0(a0), a1(a1), a2(a2)
	{
	}
	...
	void Instruction::set(int opcode, Argument a0, Argument a1, Argument a2)
	{
		this->opcode = opcode;
		this->a0 = a0;
		this->a1 = a1;
	...
	void Instruction::translatePlaceholder(int placeholderStackStart, int translation)
	{
	...
	void Instruction::updateJumpTarget(int before, int after)
	{
	...
	void Instruction::execute(Stack stack)
	{
		switch (opcode)
		{
		case Opcodes::ADD:
	...
	bool Instruction::Equals(Instruction& instruction)
	{
		if (this->getOpcode() != instruction.getOpcode())
			return false;

		for (int i = 0; i < 3; i++)
		{
			if (this->getArgument(i).i != instruction.getArgument(i).i)
				return false;
		}
